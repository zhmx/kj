...
if not g_ClientServer then 
	print("g_ClientServer");
	g_ClientServer = CServer.new();
	g_ClientServer:StartListen(Config.login.client.port);
end 
...
function ClientServer_ON_ACCEPT(connector)
	...
	g_Client_connectors[iAddress] = {connector = connector, ip = ip, iAddress = iAddress};
end 
function ClientServer_ON_DATA(iAddress, msgId, data)
	...
	local ret = logic[proto](g_Client_connectors[iAddress], request_recv);
	if ret then 
...
function ClientServer_ON_DISCONNECT(iAddress)
	...
	-- 触发掉线
	xpcall(function() Login.OnDisconnect(iAddress); end, function (err)
